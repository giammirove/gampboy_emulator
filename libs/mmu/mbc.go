...
var RAM [_RAM_END - _RAM_START + 1]byte
...
func InitMBC() {
	...
	// RAM_BANKS = make([]byte, headers.GetRamBankNumber())
...
func loadMemory() {
	...
	if err != nil || len(memory) == 0 {
		...
		RAM_BANKS = make([]byte, _RAM_BANK_SIZE*headers.GetRamBankNumber())
		return
	}
	RAM_BANKS = memory
...
func SaveMemory() {
	if save_needed {
		log.Printf("!! Saving ... \n")
		err := ioutil.WriteFile(rom_memory_path, RAM_BANKS, 0666)
		if err != nil {
...
func WriteToRomMemory(addr uint, value uint8) {
	...
	if addr >= _ROM_BANK_NUMBER_START && addr <= _ROM_BANK_NUMBER_END {
		...
		if rom_bank > headers.GetRomBankNumber() {
			rom_bank = rom_bank & headers.GetRomBankBits()
		}
...
func ReadFromRamMemory(addr uint) uint8 {
	...
	if addr >= _ERAM_START && addr <= _ERAM_END {
		if headers.IsMBC3() {
			...
			return RAM_BANKS[addr-_ERAM_START+ram_bank*_RAM_BANK_SIZE]
		}
